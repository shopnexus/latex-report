...
\rowcolor{rowgray} Voice Rephraze & STT qua PhoWhisper-large (local) + sinh mô tả qua GPT-4o-mini, 5+ phong cách viết \\
...
Dịch vụ Voice Rephraze sử dụng PhoWhisper-large (VinAI) chạy local cho kết quả
nhận dạng tiếng Việt chính xác, kể cả với giọng vùng miền (Bắc, Trung, Nam).
Việc chạy local loại bỏ phụ thuộc cloud API cho STT, giảm chi phí vận hành
và latency mạng. Module sinh mô
...
\item
  \textbf{Phụ thuộc cloud cho sinh mô tả:} Module sinh mô tả sản phẩm sử dụng
  OpenAI GPT-4o-mini API, yêu cầu kết nối internet và phát sinh chi phí vận hành.
  Module STT (PhoWhisper) đã chạy local nhưng toàn bộ pipeline vẫn cần internet
  cho bước sinh mô tả.
\item
