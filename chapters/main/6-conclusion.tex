...
PhoWhisper-large (STT local) và GPT-4o-mini (sinh mô tả) tạo ra mô tả sản phẩm
...
\begin{itemize}
\item \textbf{Thay thế GPT-4o-mini bằng LLM local:} Triển khai mô hình ngôn ngữ
  local (ví dụ: Vistral, Qwen2) để sinh mô tả sản phẩm, loại bỏ hoàn toàn
  phụ thuộc cloud API và chi phí vận hành OpenAI. Kết hợp với PhoWhisper-large
  đã chạy local, toàn bộ pipeline sẽ hoạt động offline.
