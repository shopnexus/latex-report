...
Client gửi text mô tả sản phẩm qua POST request, input được validate bằng
Pydantic schema để đảm bảo đúng format. Text sau đó được tokenize (tối đa
512 tokens) qua AutoTokenizer của HuggingFace, rồi đưa qua forward pass
để sinh logits cho 32 danh mục. Hàm softmax chuyển logits thành xác suất,
từ đó trích xuất top-K predictions kèm theo category ID, tên danh mục,
tên hiển thị và confidence score. Response trả về kèm inference time để
theo dõi hiệu năng.

Model được load một lần duy nhất khi khởi động qua FastAPI Lifespan Context
Manager, tự động phát hiện GPU (fallback về CPU nếu không có) và đặt ở chế
độ evaluation để tắt dropout và batch normalization tracking, tối ưu tốc
độ suy luận.
